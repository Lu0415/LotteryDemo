...
public class TestPanelController : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        if (groupRT != null)
        {
            ...
            // 計算顯示資料
            LotteryInfoController.SharedInstance.InitLotteryInfoData();
        }
    }
    ...
    public void CallBackForInitFruitItem(string[] dataArray, Vector3[] pointArray, float itemH, float itemW)
    {
        rewardTransArr = new Transform[pointArray.Length];
        rewardCellArr = new LotteryCell[pointArray.Length];
        ...
        foreach (var point in pointArray)
        {
            ...
            newFruit.name = dataArray[index];
            newFruit.transform.SetParent(panelTransform);
            newFruit.SetActive(true);

            RectTransform rt = newFruit.GetComponent<RectTransform>();
            ...
            rewardCellArr[index] = rewardTransArr[index].GetComponent<LotteryCell>();
    ...
    public void CallBackForStartTheLottery()
    {
        ...
        if (!IsOnClickPlaying)
        {
            var reward = LotteryInfoController.SharedInstance.sampleChar[UnityEngine.Random.Range(0, LotteryInfoController.SharedInstance.sampleChar.Length - 1)];
